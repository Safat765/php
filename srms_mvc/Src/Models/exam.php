<?php
    ...
    class examModel
    {
        ...
        {
            $con = $this->dbConnection();
            $sql = "UPDATE `exam` SET `course_id`='$course_id',`exam_title`='$exam_title',`department_id`='$department_id',`semester`='$semester',`credit`='$credit',`exam_type`='$exam_type',`marks`='$marks',`instructor_id`='$instructor_id' WHERE `exam_id`= '$exam_id'";
            $result = mysqli_query($con, $sql);
            return $result;
        }     
